...
export function createWorld(host: HTMLDivElement, onPick: (id: string) => void, onHover: (text:string) => void, onStationChange: (station:number) => void, onZoneChange:(zone:string)=>void) {
 ...
 const soup=cylinder(1.015,1.015,.035,sauces[0].color,-.12,1.71,0,potGroup);(soup.material as THREE.MeshStandardMaterial).roughness=.18;(soup.material as THREE.MeshStandardMaterial).metalness=.1;for(const x of [-1.42,1.18])box(.45,.14,.27,'#333a31',x,1.6,0,potGroup);
 ...
 for(const x of [-1.73,-1.59]){const resting=cylinder(.016,.022,.9,'#715035',x,.83,.75,support);resting.rotation.x=Math.PI/2;}
 ...
 const ladle=new THREE.Group();potGroup.add(ladle);const handle=cylinder(.022,.027,1.5,'#a3a197',0,.7,0,ladle,.8);handle.rotation.z=-.24;const spoon=mesh(new THREE.SphereGeometry(.16,16,10),'#c3b9a4',-.17,0,0,ladle,.85);spoon.scale.set(1,.22,1.3);ladle.visible=false;
 ...
 const transfers:{object:THREE.Group;from:THREE.Vector3;start:number}[]=[];let stirTime=-10;let stirAngle=0;
 ...
 function stir():void{stirTime=performance.now()/1000;stirAngle+=.7;}
 ...
 const raycaster=new THREE.Raycaster();const pointer=new THREE.Vector2();let stirDrag=false,lastStir=0,lastHint='';
 ...
 function targetAction():{id:string;label:string}{
  ...
  if(potHit){if(stage==='pick'){if(potParked&&potHit.distance<6)return{id:'pickup-pot',label:'Your pot · click to carry it to the buffet, or walk to the chair to sit'};if(!potParked)return{id:'',label:'Carry your pot to Table 04 · walk around the table to your chair'};return{id:'',label:''};}if(stage==='cook')return{id:isReady?'ready':cooking?'stir':'',label:cookingPrompt(cooking,temperature,isReady)};const id=ancestor(potHit.object,'ingredient');return{id:id?'bite:'+id:'bite',label:id?(ingredients.find(i=>i.id===id)?.name+' · click to taste'):'Click to take a bite'};}
 ...
 const down=(e:PointerEvent)=>{
  ...
  cast(e);stirDrag=stage==='cook'&&cooking&&!isReady&&targetAction().id==='stir';
 };
 const move=(e:PointerEvent)=>{
  ...
  if(drag){if(stirDrag){if(performance.now()-lastStir>140){lastStir=performance.now();onPick('stir');}}else{orbit-=dx*.003;pitch=Math.max(-.3,Math.min(.3,pitch+dy*.002));}}
 };
 const up=(e:PointerEvent)=>{
  if(!drag||e.pointerId!==activePointer||e.button!==dragButton)return;
  const secondary=dragButton===2;const moved=Math.hypot(e.clientX-startX,e.clientY-startY)>8;endDrag();
  if(secondary){e.preventDefault();onHover('');return;}
  if(moved)return;const r=renderer.domElement.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)return;cast(e);const hit=targetAction();if(hit.id)onPick(hit.id);
 };
 ...
 soup.rotation.z=-potGroup.rotation.z*.6;soup.scale.y=1+Math.sin(time*(2+boilIntensity*5))*.055*boilIntensity;bubbles.forEach((b,i)=>{const phase=(time*(.25+boilIntensity*1.9+i%3*.12)+i*.19)%1;b.scale.setScalar(Math.sin(phase*Math.PI)*(.35+boilIntensity*.9));b.position.y=1.74+phase*.065*boilIntensity;});
 ...
 const stirring=cooking&&time-stirTime<1.2;ladle.visible=stage==='cook'&&cooking;ladle.position.set(-.12+Math.cos(stirAngle)*.6,1.78,Math.sin(stirAngle)*.6);ladle.rotation.z=-.35;if(stirring){stirAngle+=dt*4;potFood.rotation.y+=dt*.55;oil.rotation.y+=dt*.4;}
 ...
 potFood.children.forEach((c,i)=>{c.position.y=1.76+(i%3)*.025+Math.sin(time*(1.2+boilIntensity*4)+i)*.045*boilIntensity;c.rotation.z=Math.sin(time*2+i)*.06*boilIntensity;});if(location.hostname==='localhost'){renderer.domElement.dataset.playerX=camera.position.x.toFixed(2);renderer.domElement.dataset.playerZ=camera.position.z.toFixed(2);renderer.domElement.dataset.potParked=String(potParked);}
